...
class WP_Test_MslsAdminIcon extends Msls_UnitTestCase {
	...
	function test_create_method() {
		...
		$this->assertInstanceOf( 'AdminIcon', MslsAdminIcon::create() );

		$obj = new MslsAdminIcon( 'post' );

		$this->assertInstanceOf( 'AdminIcon', $obj->set_path() );
		$this->assertInstanceOf( 'AdminIcon', $obj->set_language( 'de_DE' ) );
		$this->assertInstanceOf( 'AdminIcon', $obj->set_src( '/dev/german_flag.png' ) );

		$this->assertEquals( '<img alt="de_DE" src="/dev/german_flag.png" />', $obj->get_img() );
		$this->assertInternalType( 'string', $obj->get_edit_new() );

		$this->assertInstanceOf( 'AdminIcon', $obj->set_href( $post_id ) );
		...
		$this->assertInstanceOf( 'AdminIcon', $obj->set_href( 0 ) );
